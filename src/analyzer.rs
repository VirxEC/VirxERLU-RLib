...
impl Analyzer {
    ...
    pub fn target(&self, ball: &Ball, car: &Car, shot_vector: Vec3A, time_remaining: f32, slice_num: usize) -> dubins_paths::Result<TargetInfo> {
        ...
        } else if offset_target.z < car.max_double_jump_height {
        ...
        let (jump_time, end_distance) = match shot_type {
            ShotType::GROUND => {
                let distance = 320.;
                (
                    None,
            ...
            ShotType::JUMP => {
                ...
                (Some(time), time * self.get_max_speed(car, slice_num) + 128.)
            }
            ShotType::DOUBLE_JUMP => {
                let time = car.double_jump_time_to_height(self.gravity, offset_target.z - car.hitbox.height / 2.);
        ...
        let q0 = PosRot::new(flatten(car_location), car.landing_yaw);
